...
def calculate_prob_cond(array, target):
    ...
    for i in range(len(counter)):
        if counter[i] == 0:
            for row in range(len(unique_array)):
                for col in range(len(unique_array[row])):
                    if unique_list[i] == unique_array[row][col]:
                        counter[i] = 0.1 / (len(unique_array[row]))

    return unique_list, counter


def laplace_likelihood(training_set):
